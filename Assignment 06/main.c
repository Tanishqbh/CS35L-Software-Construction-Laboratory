...
void* func(void* input)
{
  ...
  const double subsample_dx = halfSamples ? pixel_dx / ((double)halfSamples*2.0)
    : pixel_dx;
  const double subsample_dy = halfSamples ? pixel_dy / ((double)halfSamples*2.0)
    : pixel_dy;
...
main( int argc, char **argv )
{
    ...
    //create threads
    pthread_t* tid_container = (pthread_t*)malloc(nthreads * sizeof(pthread_t));
    int containersize = 0;
    for(int i = 0; i < nthreads; i++)
      {
	if(pthread_create(&tid_container[i], NULL, func, &containersize))
	  {
	    fprintf(stderr, "Error: thread creation unsuccessful");
	    exit(1);
	  }
	containersize++;
      }
    ...
    for(int i = 0; i < nthreads; i++)
      {
	if(pthread_join(tid_container[i], NULL))
	  {
	    fprintf(stderr, "Error: thread joining unsuccessful");
	    exit(1);
	  }
	containersize--;
      }
